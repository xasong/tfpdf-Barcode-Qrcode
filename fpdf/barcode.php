...
class Barcode {
    ...
    public function setBarcodeString($code, $type ='C128'){
        $this->setBarcode($code, $type);
    }

    public function GenerateBarcode(&$fpdf, $x='', $y='', $w='', $h='',$xres='0.5',$fontsize=15,$font = 'DejaVu',$color=array(0,0,0))
    {
        $arrcode = $this->barcode_array;
        $code = $arrcode['code'];
        $text_height = 0;
        if (($w === '') OR ($w <= 0)) {
            $w = $x - $fpdf->lMargin;
        ...
        foreach ($arrcode['bcode'] as $k => $v) {
            ...
            if ($v['t']) {
                ...
                $fpdf->Rect($xpos, $ypos, $bw, ($v['h'] * $barh / $arrcode['maxh']), 'F');
            }
            $xpos += $bw;
        }
        $txtwidth = ($arrcode['maxw'] * $xres);
        $text_height = $y+$barh+3.8+$vpadding;
        $fpdf->SetFont($font,'',$fontsize);
        //$fpdf->SetFontSize($fontsize);
        //$fpdf->SetY($text_height);
        $code_length = strlen($code);
        $txt_xpos = round($txtwidth/$code_length,4)+0.1;
        for($i=0;$i<$code_length;$i++){
            $xpos = $x+$i*$txt_xpos;
            $fpdf->Text($xpos,$text_height,$code[$i]);
        }
        //$fpdf->Cell($txtwidth,0,$code,0,0,'C');
        return true;
    }
